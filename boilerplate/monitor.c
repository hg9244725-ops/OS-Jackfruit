...
static void timer_callback(struct timer_list *t) {
    ...
    list_for_each_entry_safe(entry, tmp, &monitored_list, list) {
        ...
        if (rss < 0) {
            printk(KERN_INFO "[container_monitor] Process exited, removing container=%s pid=%d\n", entry->container_id, entry->pid);
            list_del(&entry->list);
            kfree(entry);
            continue;
        }
...
static long monitor_ioctl(struct file *f, unsigned int cmd, unsigned long arg) {
    ...
    if (cmd == MONITOR_REGISTER) {
        ...
        if (req.soft_limit_bytes > req.hard_limit_bytes) {
            printk(KERN_WARNING "[container_monitor] Invalid limits: soft > hard for container=%s\n", req.container_id);
            return -EINVAL;
        }
    ...
    /* MONITOR_UNREGISTER */
    {
        ...
        mutex_unlock(&monitored_lock);
        printk(KERN_INFO "[container_monitor] Unregistered container=%s pid=%d found=%d\n", req.container_id, req.pid, found);
